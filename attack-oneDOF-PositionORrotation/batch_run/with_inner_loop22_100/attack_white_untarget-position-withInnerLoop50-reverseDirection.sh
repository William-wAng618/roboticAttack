#!/bin/bash
#SBATCH -J 22bai-unTarget-reverse-direction
#SBATCH -t 48:00:00
...
config_file="openvla-main/attack-oneDOF-PositionORrotation/batch_run/with_inner_loop22_100/config.json"
...
innerLoop=$(jq -r '.innerLoop' $config_file)
tags="position UntargetedreverseDirection InnerLoop${innerLoop}"
tags="${tag_prefix} ${tags}"
# Run the Python script with parsed parameters
python openvla-main/attack-oneDOF-PositionORrotation/attack_white_patch_single_withInnerLoop_reverseDirection.py \
    --lr $lr \
    --maskidx $maskidx \
    --server $server \
    --device $device \
