...
def main(args):
    ...
    from openvla_attacker_untarget_withInnerLoop import OpenVLAAttacker
    ...
    for i in args.maskidx:
        target += str(i)
    name = f"{args.dataset}_{vla_path}_GA{args.accumulate}_lr{format(args.lr, '.0e')}_iter{args.iter}_warmup{args.warmup}_filterGripTrainTo1{args.filterGripTrainTo1}_target{target}_Untarget{args.guide}_inner_loop{args.innerLoop}_geometry{args.geometry}_colorjitter{args.colorjitter}_guide{args.guide}_patch_size{args.patch_size}_seed42-{exp_id}"
...
def arg_parser():
    ...
    parser.add_argument('--tags',nargs='+', default=["50x50","DoF1","LRDecay","Untargeted-guide","WarmUp","innerLoop50","single"])
    ...
    parser.add_argument('--wandb_project', default="openvla_relativeDistance_guide", type=str)
    parser.add_argument('--guide', type=str2bool, nargs='?',default=False,
                        help='add guide or not')
