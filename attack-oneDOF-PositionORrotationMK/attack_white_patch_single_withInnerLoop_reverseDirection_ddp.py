...
def get_dataloader(dataset, rank, world_size):
    ...
    return DataLoader(dataset, sampler=sampler, batch_size=8)

def main(rank, args,world_size):
    print(f"Running process {rank}")
    dist.init_process_group("nccl", rank=rank, world_size=world_size)
    print(f"2Running process {rank}")
    ...
    train_dataloader = get_dataloader(train_dataloader.dataset)
    val_dataloader = get_dataloader(val_dataloader.dataset)
    ...
    patch = torch.rand(args.patch_size).to(rank)
    openVLA_Attacker.patchattack_unconstrained(patch,train_dataloader, val_dataloader, num_iter=args.iter,
...
if __name__ == "__main__":
    ...
    if is_main_process():
        ...
        from openvla_attacker_untarget_withInnerLoop_reverseDirection_ddp import OpenVLAAttacker
        ...
        wandb.config = {"iteration": args.iter, "learning_rate": args.lr, "attack_target": args.maskidx,
                        "accumulate_steps": args.accumulate}
    world_size = 2
    os.environ["NCCL_DEBUG"] = "INFO"
    print(f"world_size:{world_size}")
    torch.multiprocessing.spawn(main, args=(args,world_size), nprocs=world_size, join=True)