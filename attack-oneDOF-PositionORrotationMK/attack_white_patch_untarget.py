...
def main(args):
    ...
    from openvla_attacker_untarget import OpenVLAAttacker
    ...
    for i in args.maskidx:
        target += str(i)
    name = f"{args.dataset}_{vla_path}_GA{args.accumulate}_lr{format(args.lr, '.0e')}_iter{args.iter}_warmup{args.warmup}_filterGripTrainTo1{args.filterGripTrainTo1}_target{target}-{exp_id}"
...
def arg_parser():
    ...
    parser.add_argument('--maskidx',default='1,3,5', type=list_of_ints)
