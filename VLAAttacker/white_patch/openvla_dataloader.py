...
def get_dataloader_forvis(batch_size,server,dataset,sample_num):
    # vla_path: str = "openvla/openvla-7b"
    data_root_dir = Path(f"{server}/openvla-main/dataset")
    # dataset_name = "bridge_orig"
    if dataset == "bridge_orig":
    ...
    sample_indices = random.sample(range(len(5000)), sample_num)
    # subset_dataset = Subset(vla_dataset_train, sample_indices)
    sampled_data = []
    for i, data in enumerate(iter(vla_dataset_train)):
        if i in sample_indices:
            sampled_data.append(data)
            if len(sampled_data) == sample_num:
                break
    sampled_dataset = SampledDataset(sampled_data)
